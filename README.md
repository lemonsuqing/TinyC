...
### 逻辑的进阶：短路求值 (Logical Operators)
*   **新能力**: 支持逻辑与 (`&&`) 和逻辑或 (`||`) 运算符，实现了符合 C 标准的复杂条件判断。
*   **核心实现**:
    *   **短路求值 (Short-circuit Evaluation)**: 摒弃了简单的位运算指令，而是通过生成条件跳转指令 (`je`, `jne`) 来控制执行流。例如在计算 `A && B` 时，若 `A` 为假，则直接跳过 `B` 的计算。这不仅提高了效率，还保证了如 `ptr && *ptr` 这样代码的安全性。

### 语法糖与便利性 (Syntax Sugar & Usability)
*   **新能力**:
    *   **For 循环**: 支持标准的 C 语言 `for (init; cond; inc) { body }` 结构。
    *   **注释**: 支持单行注释 (`//`) 和多行注释 (`/* ... */`)，Lexer 会在读取 Token 时自动忽略注释内容，提升了代码的可读性。
*   **技术细节**:
    *   **控制流转换**: 将 `for` 循环映射为 "初始化 -> 标签 -> 条件检查 -> 循环体 -> 增量 -> 跳转" 的汇编逻辑，并在 Parser 中特殊处理了初始化和增量部分的赋值表达式。
    *   **递归符号扫描 (Recursive Scope Scan)**: 重构了代码生成器的局部变量分配逻辑。从仅扫描顶层改为**递归扫描**整个 AST 树，确保嵌套在 `for` 循环初始化部分或 `if` 代码块深处的变量声明都能被正确识别并在栈上分配空间。

### 循环的补完：跳转控制 (Break & Continue)
*   **新能力**: 支持在循环中使用 `break` 终止循环，或使用 `continue` 跳过当前迭代。
*   **技术细节**:
    *   **上下文管理**: 在代码生成器中引入状态变量，记录当前所处循环的 "End Label" (用于 break) 和 "Increment/Start Label" (用于 continue)。
    *   **嵌套支持**: 在进入新的循环前保存旧的 Label ID，退出时恢复，从而完美支持多层循环嵌套。

### 工程化迈进：文件读取与命令行支持 (File I/O & CLI)
*   **新能力**: 编译器不再局限于编译硬编码的字符串，而是能够读取外部 `.c` 文件，真正成为一个命令行工具。
*   **核心实现**:
    *   实现了 `read_file` 函数，利用 `fseek`/`ftell`/`fread` 一次性读取完整源码文件。
    *   在 `main` 函数中添加了 `argc`/`argv` 解析逻辑，支持通过命令行参数指定输入文件（如 `./tinyc test.c`）。
    *   **测试解耦**: 将测试用例从编译器源码中剥离到独立的 `tests/` 目录，使得编写和维护复杂的测试代码变得更加轻松。


## 后续计划：
### 类型系统的扩展 (Type System)
这是最难的一步，标志着你的编译器走向成熟。
- 字符类型 (char):  - - - **(已完成✅)**

    目前只有 8 字节的 int。支持 1 字节的 char 需要你处理 x86-64 的 al 寄存器和 movzx (零扩展) 指令。
...
大魔王级别。
    ...
    需要计算每个成员在内存中的偏移量 (Offset)。

    支持 obj.field 访问。

### 自举 (Self-Hosting) —— 终极目标
尝试用 TinyC 编写 TinyC 自己的源代码。

这需要预处理器 (#include, #define) 的支持。